# risk-agent-bank SAM Plugin

Analyzes customer history and transactions to detect possible fraud.

This is a plugin for the Solace Agent Mesh (SAM).

## Configuration (`config.yaml`)

The `config.yaml` in this plugin serves as a template. When you use `sam plugin add <component_name> --plugin risk-agent-bank`, the following placeholders in the YAML structure will be replaced with variations of `<component_name>`:
...
```bash
sam plugin add <your-new-component-name> --plugin risk-agent-bank
```
This will create a new component configuration at `configs/plugins/<your-new-component-name-kebab-case>.yaml`.# risk-agent-bank
# risk-agent-bank
